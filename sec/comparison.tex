...
\section{The Cartan--Serre comparison} \label{s:the cartan-serre comparison map}
...
The \textit{simplicial singular complex} functor $\sSing \colon \Top \to \sSet$ is defined for a topological space $Z$ by
\[
\sSing(Z)_n = \Top(\gsimplex^n, Z).
\]

\subsubsection{Algebraic aspects}

The functor of (\textit{normalized}) \textit{chains} $\schains \colon \sSet \to \Ch$ is the Yoneda extension of the composition of the functor of cellular chains with the functor $\gsimplex^{(-)}$ define above.
...
\[
\left[v_0, \dots, v_p \right] \ast \left[v_{p+1}, \dots, v_q\right] = \begin{cases} (-1)^{p+|\sigma|} \left[v_{\sigma(0)}, \dots, v_{\sigma(q)}\right] & \text{ if } v_i \neq v_j \text{ for } i \neq j, \\
...
\subsection{The cellular maps} \label{ss:cs collapse map}

\anibal{say something}

\subsubsection{Cartan-Serre projection}

The \textit{Cartan-Serre projection} is the cellular map $\cs \colon \gcube^n \to \gsimplex^n$ defined by
\[
\cs(x_1, \dots, x_n) = (x_1,\ x_1 x_2, \, \dots \, , \ x_1 x_2 \dotsm x_n).
\]
It induces for any topological $Z$ a natural chain map
\[
\begin{tikzcd} [column sep=small, row sep=0]
	\CS_Z \colon &[-25pt] \sSchains(Z) \arrow[r] & \cSchains (Z) \\
	& (\gsimplex^n \to Z) \arrow[r, mapsto] & (\gcube^n \xra{\cs} \gsimplex^n \to Z).
\end{tikzcd}
\]
The map $\cs$ was considered in \cite[p. 442]{serre1951homologie} where it was stated that $\CS_Z$ is a natural quasi-isomorphisms of coalgebras for any $Z$.
We remark that in this reference the topological $n$-simplex is modeled as subset of $\R^{n+1}$ instead of $\R^n$.
...
\subsubsection{The Eilenberg--Zilber subdivision}

For any permutation $\sigma \in \Sym_n$ we have an inclusion $\incl_\sigma \colon \gsimplex^n \to \gcube^n$ defined by
\[
(x_1, \dots, x_n) \mapsto (x_{\sigma(1)}, \dots, x_{\sigma(n)}).
\]
If $e$ is the identity we denote $\incl_{e}$ simply as $\incl$ and notice that it satisfies $\cs \circ \incl = \id$.
The maps $\set{\incl_\sigma}_{\sigma \in \Sym_n}$ define a subdivision of $\gcube^n$ making it isomorphic to $\bars[\big]{\scube{n}}$ in $\CW$.
...
\]
For any topological space $Z$ we have a quasi-isomorphism
\[
\begin{tikzcd} [column sep=small, row sep=0]
	\EZ_Z \colon &[-25pt] \cSchains(Z) \arrow[r] & \sSchains (Z) \\
	& (\gcube^n \to Z) \arrow[r, mapsto] &
	\sum\limits_{\mathclap{\sigma \in \Sym_n}} \sign(\sigma) \big( \gsimplex^n \xra{\incl_\sigma} \gcube^n \to Z \big).
\end{tikzcd}
\]
...
\anibal{rescued:
An easy consequence of the compatibility of $\cs$ and $\proj$ with respect to cubical and simplicial coface maps is that $\CS$ and $\proj_\ast \circ \subdivide$ agree, since they both send the top dimensional generator $[0,1]^{\ot n}$ to $[0, \dots, n]$.}
...
\big( \scube{m} \xra{\proj} \simplex^m \xra{\yoneda(\sigma)} \simplex^n \big).
\]
...
\begin{proof}
	...
	Since $\CS = \proj_\ast \circ \subdivide$ is a chain map, we have
	\begin{align*}
	\CScomp \big( \bd^\simplex \id_{[n]} \big) \defeq
	\bd^\simplex \circ \CS =
	\CS \circ \, \bd^\cube \defeq
...
\begin{example}
	We illustrate the need to restrict to $\USL$ by providing a simple example showing that $\CS$ does not preserve $\M$-structures.
	From it is easy to construct others showing $\CS$ does not preserve the $\UM$-structures either.

	Since $\CS \big( [0] \ot [0,1] \big) = 0$ and
	\begin{align*}
	\big( [1] \ot [1] \big) \ast \big( [0] \ot [0,1] \big) &=
	- \big( [0,1] \ot [0,1] \big), \\
	- \CS \big( [0,1] \ot [0,1] \big) & = - [0,1,2]
	\end{align*}
	we have
	\[
	\CS \big( [1] \ot [1] \big) \ast \CS \big( [0] \ot [0,1] \big) \neq \CS \Big( ([1] \ot [1]) \ast ([0] \ot [0,1]) \Big).
	\]
...
\begin{lemma} \label{l:main comparison}
	The chain map $\CS \colon \chains(\cube^n) \to \chains(\simplex^n)$ induced by the Cartan--Serre collapse map is a quasi-isomorphism of $\USL$-coalgebras for every $n \in \N$.
\end{lemma}

\begin{proof}[Proof of \cref{t:main comparison}]
	It suffices to prove this for $Y = \simplex^n$.
	Since $\CS = \proj_\ast \circ \subdivide$ is a morphism of $\USL$-coalgebras (\cref{l:main comparison}), for any $\Gamma \in \USL(r)$ we have
	\begin{align*}
	\CScomp^{\ot r} \! \big( \Gamma(\id_{[n]}) \big) \defeq
	\Gamma \circ \CS =
	\CS^{\ot r} \! \circ \, \Gamma \defeq
...
\]
is the chain map obtained by precomposing a singular simplex with the Cartan--Serre collapse map $\cs \colon \gcube^n \to \gsimplex^{\!n}$.
...
We begin by verifying that $\CS$ is compatible with respect to the Serre and Alexander--Whitney coalgebra structures.

\begin{lemma}
	The chain map $\CS$ is a quasi-isomorphism of coalgebras.
\end{lemma}
...
\begin{proof}
	...
	\end{align*}
	By \cref{l:cs collapse chain map explicit}, the summands above not sent to $0$ by $\CS \ot \CS$ are those basis elements for which $x_i^{(\lambda)} = [0]$ implies $x_j^{(\lambda)} = [0]$ for all $i < j$.
...
\begin{proof}
	...
	Since neither of $x$ nor $y$ have a factor $[0]$ in them, \cref{l:cs collapse chain map explicit} implies that the vertex $[n]$ is in both $\CS(x)$ and $\CS(y)$, which implies $\CS(x) \ast \CS(y) = 0$ as claimed.
